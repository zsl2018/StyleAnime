...
def get_colorization_data(data_raw, ab_thresh=5., p=.125, num_points=None):
    ...
    if(5. > 0): # mask out grayscale images
        thresh = 1.*5./110
        # print(torch.max(data['B'], dim=3)[0])
        # print(torch.min(torch.min(data['B'], dim=3)[0], dim=2)[0])
    ...
    return add_color_patches_rand_gt(data, p=p, num_points=num_points)

#def add_color_patches_rand_gt(data,opt,p=.125,num_points=None,use_avg=True,samp='normal'):
def add_color_patches_rand_gt(data,p=.125,num_points=None,use_avg=True,samp='normal'):
